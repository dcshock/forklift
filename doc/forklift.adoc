...
Requirement, install sbt (the build framework used by Forklift).

Clone the forklift repository.
...
$ sbt clean compile "run -monitor1 ~/forklift/deploy -builtinActiveMQ"

### Write Your First Consumer

```
import forklift.decorator.*;
