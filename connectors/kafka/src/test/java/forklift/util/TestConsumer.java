...
public final class TestConsumer<K, V> implements Consumer<K, V> {
    ...
    private Queue<Runnable> afterPollActions = new LinkedBlockingQueue<>();
        ...
        try {
            ...
            final Map<TopicPartition, List<ConsumerRecord<K, V>>> recordMap = assignment.stream()
                .flatMap(partition -> {
                    ...
                    final long fetchPosition = fetchPositions.getOrDefault(partition, 0L);
            ...
            final Runnable action = afterPollActions.poll();
    ...
    */
    ...
    public void runOnceAfterPoll(Runnable action) {
        afterPollActions.add(action);
    }
    ...
    /*
      ...
      -----
    */
    public void rebalancePartitions(Collection<TopicPartition> partitions) {
        listener.onPartitionsRevoked(partitions);

        /* reset fetch positions back to committed positions */
        partitions.forEach(partition -> {
            final long committedOffset = commitPositions.getOrDefault(partition, 0L);
            fetchPositions.put(partition, committedOffset);
        });
