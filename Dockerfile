...
# Add forklift server
WORKDIR /tmp
#ADD https://github.com/dcshock/forklift/releases/download/0.15/forklift-server-0.15.zip forklift.zip
ADD server/target/universal/forklift-server-0.15.zip forklift.zip
RUN yes | unzip -d /usr/local forklift.zip
RUN ln -s /usr/local/forklift-server-0.15 /usr/local/forklift
